...
def test_list_directory_not_found():
    tool = ListDirectoryTool()
    result = tool(path="/nonexistent/path")
...
def test_find_files(tmp_path):
    ...
    tool = FindFilesTool()
    result = tool(directory=str(tmp_path), pattern="*.py")
...
def test_find_files_no_match(tmp_path):
    tool = FindFilesTool()
    result = tool(directory=str(tmp_path), pattern="*.xyz")
...
def test_write_file(tmp_path):
    ...
    result = tool(path=path, content="hello")
    assert "成功" in result
...
def test_write_file_respects_allowed_dirs(tmp_path):
    ...
    result = tool(path=str(other / "x.txt"), content="data")
...
def test_move_file(tmp_path):
    ...
    result = tool(src=str(src), dst=dst)
    assert "成功" in result
